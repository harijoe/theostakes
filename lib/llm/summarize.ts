...
const SYSTEM_PROMPT = `You are an assistant that summarizes YouTube video transcripts.
You must respond with valid JSON only — no markdown, no prose, no code fences.
The JSON must match this exact shape:
{
  "oneLiner": "One sentence summarizing the video.",
  "keyTakeaways": [
    { "text": "A key takeaway.", "timestamp": "mm:ss" }
  ],
  "categories": ["category1", "category2"]
}
Rules:
- oneLiner: max 120 characters
- keyTakeaways: 3–7 items, each with a short text and an optional timestamp if inferable from the transcript
